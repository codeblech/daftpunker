...
def youtube_video_info(video_id, api_key):
    ...
    video_info = {
        'title': response['items'][0]['snippet']['title'] if 'items' in response and response['items'] else '',
        'description': response['items'][0]['snippet']['description'] if 'items' in response and response['items'] else '',
        'duration': response['items'][0]['contentDetails']['duration'] if 'items' in response and response['items'] else '',
        'views': int(response['items'][0]['statistics']['viewCount']) if 'items' in response and response['items'] else 0,
        'genre': response['items'][0]['snippet']['categoryId'] if 'items' in response and response['items'] else '',
        'upload_date': response['items'][0]['snippet']['publishedAt'] if 'items' in response and response['items'] else '',
...
def main():
    ...
    youtube_video_url = 'https://www.youtube.com/watch?v=9z-Mh9Qeinw&ab_channel=bonnietylerVEVO'
